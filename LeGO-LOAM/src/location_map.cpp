...
class MapManager {
 public:
  ...
  Eigen::Matrix4f GetPoseInMap(const pcl::PointCloud<PointType>::Ptr& points) {
 ...
 public:
  void performLoopClosure() {
    if (new_trajecto_flag) {
      ...
      auto pose = VectorToAffined(std::vector<float>(
          std::begin(transformAftMapped), std::end(transformAftMapped)));
      Eigen::Matrix4f expect_pose =
          pose.matrix().template cast<float>();  // Eigen::Matrix4f::Identity();
      ...
      if (icp.hasConverged() == false ||
        ...
        std::cout << "LoctionMapOptimization  not found constrait" << std::endl;
...
int main(int argc, char** argv) {
  ...
  std::thread visualizeMapThread(
      &LoctionMapOptimization::visualizeGlobalMapThread, &MO);
  ros::NodeHandle nh;
